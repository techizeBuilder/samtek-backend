...
export const seedPackagingDispatch = async () => {
  const user = await User.findOne({ companyId: { $exists: true, $ne: null } }).lean();
  if (!user) return { success: false, message: 'No user with companyId found.' };
  ...
  await PackagingJob.deleteMany({ company: companyId }).catch(() => {});
  await DispatchOrder.deleteMany({ company: companyId }).catch(() => {});
  ...
  const qcTeam = await ProductionTeam.findOne({ company: companyId, name: /QC/i }).lean();

  // ── Inject 5 more Completed production orders (Final Testing Approved) ────
  // These will show in the Packaging Queue
  let newOrders = [];
  const rOrders = await tryOp('Extra completed production orders (5)', () =>
    ProductionOrder.insertMany([
      {
  ...
  if (rOrders.ok) newOrders = rOrders.result;

  // Also grab the existing ORD-2026-001 (already Completed + QC Approved from seedRDProduction)
  const ord001 = await ProductionOrder.findOne({ company: companyId, orderId: 'ORD-2026-001' }).lean();
  ...
  if (ord008) {
    jobDefs.push({
      ...
      status: 'Pending',
      checklist: { allPartsIncluded: false, accessoriesIncluded: false, manualIncluded: false, safetyPackingCompleted: false },
  ...
  if (ord007) {
    jobDefs.push({
      ...
      checklist: { allPartsIncluded: true, accessoriesIncluded: true, manualIncluded: true, safetyPackingCompleted: false },
  ...
  if (ord006) {
    jobDefs.push({
      ...
      checklist: { allPartsIncluded: true, accessoriesIncluded: true, manualIncluded: true, safetyPackingCompleted: true },
  ...
  const rJobs = await tryOp(`Packaging Jobs (${jobDefs.length})`, () => PackagingJob.insertMany(jobDefs));
  results.push(rJobs);
  if (rJobs.ok) jobs = rJobs.result;

  // Find jobs by jobId
  const jobMap = {};
  for (const j of jobs) jobMap[j.jobId] = j;
  ...
  const rDispatch = await tryOp(`Dispatch Orders (${dispatchDefs.length})`, () => DispatchOrder.insertMany(dispatchDefs));
...
function completedProcesses(qcTeam, startDate, endDate) {
  return [
    ...
    { step: 'Final Testing', type: 'In-House', status: 'Completed', assignedTeam: qcTeam?._id || null, startDate: daysAgo(2), endDate: daysAgo(1), qcStatus: 'Approved', qcBy: 'Nalini R', qcDate: daysAgo(1), notes: 'All parameters within specification. Machine approved for dispatch.', reworks: [] },
  ];
}
