...
export const seedRDProduction = async () => {
  const user = await User.findOne({ companyId: { $exists: true, $ne: null } }).lean();
  if (!user) {
  ...
  await Promise.all([
    RDMachine, RDBOM, RDPrototype, RDChangeRequest, RDToolProcess,
    RDQualityParam, RDDocument, ProductionTeam, ProductionOrder,
  ].map(M => M.deleteMany({ company: companyId }).catch(() => {})));

  // ── R&D Machines ──────────────────────────────────────────────────────────
  let machines = [];
  const r0 = await tryOp('RD Machines (5)', () => RDMachine.insertMany([
    {
      code: 'SAM-001',
  ...
  results.push(r0);
  if (r0.ok) machines = r0.result;

  const [m1, m2, m3, m4] = machines;

  // ── BOMs ──────────────────────────────────────────────────────────────────
  if (m1) {
    results.push(await tryOp('BOMs (4)', () => RDBOM.insertMany([
      {
        machine: m1._id,
  ...
  // ── Prototypes ────────────────────────────────────────────────────────────
  if (m1 && m2 && m3) {
    results.push(await tryOp('Prototypes (3)', () => RDPrototype.insertMany([
      {
  ...
  // ── Change Requests ───────────────────────────────────────────────────────
  if (m1 && m2 && m3) {
    results.push(await tryOp('Change Requests (3)', () => RDChangeRequest.insertMany([
      {
  ...
  // ── Tool Processes ────────────────────────────────────────────────────────
  if (m1 && m2) {
    results.push(await tryOp('Tool Processes (2)', () => RDToolProcess.insertMany([
      {
        machine: m1._id,
        tools: [
  ...
  // ── Quality Params ────────────────────────────────────────────────────────
  if (m1 && m2) {
    results.push(await tryOp('Quality Params (2)', () => RDQualityParam.insertMany([
      {
  ...
  // ── Documents ─────────────────────────────────────────────────────────────
  if (m1 && m3) {
    results.push(await tryOp('Documents (6)', () => RDDocument.insertMany([
      {
        machine: m1._id, machineCode: m1.code, machineName: m1.name,
  ...
  // ── Production Teams ──────────────────────────────────────────────────────
  let teams = [];
  const rTeams = await tryOp('Production Teams (4)', () => ProductionTeam.insertMany([
    {
  ...
  results.push(rTeams);
  if (rTeams.ok) teams = rTeams.result;

  const [tAlpha, tBeta, tPaint, tQC] = teams;

  // ── Production Orders ─────────────────────────────────────────────────────
  results.push(await tryOp('Production Orders (5)', () => ProductionOrder.insertMany([
    {
      ...
      processes: [
        ...
        { step: 'Fabrication', type: 'In-House', status: 'Completed', assignedTeam: tBeta?._id || null, startDate: daysAgo(22), endDate: daysAgo(16), qcStatus: 'Approved', qcBy: 'Nalini R', qcDate: daysAgo(16), notes: '', reworks: [] },
        { step: 'Assembly', type: 'In-House', status: 'Completed', assignedTeam: tAlpha?._id || null, startDate: daysAgo(15), endDate: daysAgo(10), qcStatus: 'Approved', qcBy: 'Nalini R', qcDate: daysAgo(10), notes: '', reworks: [] },
        { step: 'Painting', type: 'In-House', status: 'Completed', assignedTeam: tPaint?._id || null, startDate: daysAgo(9), endDate: daysAgo(7), qcStatus: 'Approved', qcBy: 'Deepak M', qcDate: daysAgo(7), notes: 'RAL 9003 powder coat applied', reworks: [] },
        { step: 'Re-Assembly', type: 'In-House', status: 'Completed', assignedTeam: tAlpha?._id || null, startDate: daysAgo(6), endDate: daysAgo(6), qcStatus: 'Approved', qcBy: 'Nalini R', qcDate: daysAgo(6), notes: '', reworks: [] },
        { step: 'Final Testing', type: 'In-House', status: 'Completed', assignedTeam: tQC?._id || null, startDate: daysAgo(5), endDate: daysAgo(5), qcStatus: 'Approved', qcBy: 'Nalini R', qcDate: daysAgo(5), notes: 'All parameters within spec. Speed: 118 pouches/min', reworks: [] },
      ],
      materialDemands: [
    ...
    },
    {
      ...
      processes: [
        ...
        { step: 'Fabrication', type: 'In-House', status: 'Completed', assignedTeam: tBeta?._id || null, startDate: daysAgo(9), endDate: daysAgo(5), qcStatus: 'Approved', qcBy: 'Deepak M', qcDate: daysAgo(5), notes: '', reworks: [] },
        { step: 'Assembly', type: 'In-House', status: 'In Progress', assignedTeam: tAlpha?._id || null, startDate: daysAgo(4), endDate: null, qcStatus: 'Pending', qcBy: null, qcDate: null, notes: 'Heater band installation in progress', reworks: [] },
        ...
        { step: 'Final Testing', type: 'In-House', status: 'Pending', assignedTeam: tQC?._id || null, startDate: null, endDate: null, qcStatus: 'Pending', qcBy: null, qcDate: null, notes: '', reworks: [] },
      ],
      materialDemands: [
    ...
    },
    {
      ...
      processes: [
        ...
        { step: 'Fabrication', type: 'In-House', status: 'In Progress', assignedTeam: tBeta?._id || null, startDate: daysAgo(2), endDate: null, qcStatus: 'Pending', qcBy: null, qcDate: null, notes: 'Awaiting SS pipe arrival (CR-2026-002)', reworks: [] },
        { step: 'Assembly', type: 'In-House', status: 'Pending', assignedTeam: tAlpha?._id || null, startDate: null, endDate: null, qcStatus: 'Pending', qcBy: null, qcDate: null, notes: '', reworks: [] },
        { step: 'Painting', type: 'In-House', status: 'Pending', assignedTeam: null, startDate: null, endDate: null, qcStatus: 'Pending', qcBy: null, qcDate: null, notes: '', reworks: [] },
        { step: 'Re-Assembly', type: 'In-House', status: 'Pending', assignedTeam: null, startDate: null, endDate: null, qcStatus: 'Pending', qcBy: null, qcDate: null, notes: '', reworks: [] },
        { step: 'Final Testing', type: 'In-House', status: 'Pending', assignedTeam: tQC?._id || null, startDate: null, endDate: null, qcStatus: 'Pending', qcBy: null, qcDate: null, notes: '', reworks: [] },
      ],
