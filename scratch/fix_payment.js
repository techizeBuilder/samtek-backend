...
const createAccountingEntries = async (leadPayment, bankAccount) => {
  try {
    const lineItems = [];
    ...
    if (bankAccount) {
      lineItems.push({
        ...
        accountName: `${bankAccount.bankDetails?.bankName || 'Bank'} - ${bankAccount.accountName}`,
    ...
    const count = await LedgerEntry.countDocuments({ companyId: leadPayment.companyId });
...
const run = async () => {
  try {
    ...
    const payments = await LeadPayment.find({ leadCode: 'LD-0004' });
    ...
    for (const payment of payments) {
      ...
      if (payment.bankAccount) {
        bankAccount = await Account.findById(payment.bankAccount);
      }

      // Check if LedgerEntry already exists
      const existingLedger = await LedgerEntry.findOne({ referenceId: payment._id });
      if (!existingLedger) {
        console.log('No existing ledger entry found. Creating one now...');
        await createAccountingEntries(payment, bankAccount);
      } else {
