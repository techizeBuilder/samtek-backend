...
async function fixStoreFieldCorrectly() {
  try {
    ...
    const allProducts = await Item.find({}).select('name code store');
    ...
    const myNewProductsWithoutStore = productsWithoutStore.filter(p => 
      newProductCodes.includes(p.code)
    );
    ...
    if (myNewProductsWithoutStore.length > 0) {
      ...
      // Show final status
      const finalNewProducts = await Item.find({ 
