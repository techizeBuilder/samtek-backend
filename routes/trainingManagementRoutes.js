...
router.post('/stage-candidate', authenticateToken, authorizeRoles(...ALL_MANAGEMENT), stageCandidate);

// Get the active trainee list for the table
router.get('/trainees', authenticateToken, authorizeRoles(...ALL_MANAGEMENT), getActiveTrainees);

// Get full details for a specific trainee (Progress, Tests, Modules)
router.get('/trainees/:id', authenticateToken, authorizeRoles(...ALL_MANAGEMENT), getTraineeDetails);
...
router.post('/modules', authenticateToken, authorizeRoles(...ALL_MANAGEMENT), lmsUpload.array('files', 5), createTrainingModule);

// Get all modules
router.get('/modules', authenticateToken, authorizeRoles(...ALL_MANAGEMENT), getModules);

// Update module details (Title, Category, etc.)
router.put('/modules/:id', authenticateToken, authorizeRoles(...ALL_MANAGEMENT), updateTrainingModule);

// Deactivate a module
router.delete('/modules/:id', authenticateToken, authorizeRoles(...ALL_MANAGEMENT), deactivateModule);

// Add new media to an existing module
router.post('/modules/:id/media', authenticateToken, authorizeRoles(...ALL_MANAGEMENT), lmsUpload.array('files', 5), addMediaToModule);
...
router.post('/modules/:moduleId/questions', authenticateToken, authorizeRoles(...ALL_MANAGEMENT), addQuestionToModule);

// Get all questions for a module (Shows correct answers - MANAGERS ONLY)
router.get('/modules/:moduleId/questions', authenticateToken, authorizeRoles(...ALL_MANAGEMENT), getQuestions);

// Update a specific question
router.put('/questions/:questionId', authenticateToken, authorizeRoles(...ALL_MANAGEMENT), updateQuestion);

// Delete a question
router.delete('/questions/:questionId', authenticateToken, authorizeRoles(...ALL_MANAGEMENT), deleteQuestion);
