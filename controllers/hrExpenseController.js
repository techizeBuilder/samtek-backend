import HrExpense, { HR_EXPENSE_CATEGORIES } from '../models/HrExpense.js';

const cid = (req) => req.user.companyId;

const HEAD_ROLES = ['HR-Admin', 'Hr Admin', 'Superadmin', 'Super Admin'];
...
export const getHrExpenseCategories = async (req, res) => {
  res.json({ success: true, data: HR_EXPENSE_CATEGORIES });
};
...
export const createHrExpense = async (req, res) => {
  try {
    ...
    if (!category || !HR_EXPENSE_CATEGORIES.includes(category)) {
    ...
    const expense = await HrExpense.create({
      companyId: cid(req),
      category,
      amount: amountNum,
    ...
    console.error('Error creating HR expense:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// ─── GET /api/hr-expenses ────────────────────────────────────────────────────
export const getHrExpenses = async (req, res) => {
  try {
    ...
    const [expenses, total, totalAmountAgg] = await Promise.all([
      HrExpense.find(query)
        .populate('addedBy', 'fullName username')
        .sort({ date: -1, createdAt: -1 })
        .skip(skip)
        .limit(limitNum)
        .lean(),
      HrExpense.countDocuments(query),
      HrExpense.aggregate([
        { $match: query },
    ...
    console.error('Error fetching HR expenses:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// ─── GET /api/hr-expenses/summary ───────────────────────────────────────────
export const getHrExpenseSummary = async (req, res) => {
  try {
    ...
    console.error('Error fetching HR expense summary:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};
...
export const updateHrExpense = async (req, res) => {
  try {
    const expense = await HrExpense.findOne({ _id: req.params.id, companyId: cid(req) });
    ...
    if (category !== undefined) {
      if (!HR_EXPENSE_CATEGORIES.includes(category)) {
    ...
    console.error('Error updating HR expense:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};
...
export const deleteHrExpense = async (req, res) => {
  try {
    ...
    console.error('Error deleting HR expense:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};
