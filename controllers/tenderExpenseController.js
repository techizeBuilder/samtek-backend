import TenderExpense, { TENDER_EXPENSE_CATEGORIES } from '../models/TenderExpense.js';

const cid = (req) => req.user.companyId;

const HEAD_ROLES = ['Accounts Head', 'Superadmin', 'Super Admin'];
...
// ─── GET /api/accounts/tender-expenses/categories ───────────────────────────
export const getTenderExpenseCategories = async (req, res) => {
  res.json({ success: true, data: TENDER_EXPENSE_CATEGORIES });
};

// ─── POST /api/accounts/tender-expenses ─────────────────────────────────────
export const createTenderExpense = async (req, res) => {
  try {
    const { category, amount, date, notes } = req.body;

    if (!category || !TENDER_EXPENSE_CATEGORIES.includes(category)) {
    ...
    const expense = await TenderExpense.create({
      companyId: cid(req),
      category,
      amount: amountNum,
    ...
    console.error('Error creating tender expense:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// ─── GET /api/accounts/tender-expenses ───────────────────────────────────────
export const getTenderExpenses = async (req, res) => {
  try {
    ...
    const [expenses, total, totalAmountAgg] = await Promise.all([
      TenderExpense.find(query)
        .populate('addedBy', 'fullName username')
        .sort({ date: -1, createdAt: -1 })
        .skip(skip)
        .limit(limitNum)
        .lean(),
      TenderExpense.countDocuments(query),
      TenderExpense.aggregate([
        { $match: query },
    ...
    console.error('Error fetching tender expenses:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// ─── GET /api/accounts/tender-expenses/summary ──────────────────────────────
export const getTenderExpenseSummary = async (req, res) => {
  try {
    ...
    console.error('Error fetching tender expense summary:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// ─── PUT /api/accounts/tender-expenses/:id ───────────────────────────────────
export const updateTenderExpense = async (req, res) => {
  try {
    const expense = await TenderExpense.findOne({ _id: req.params.id, companyId: cid(req) });
    ...
    if (category !== undefined) {
      if (!TENDER_EXPENSE_CATEGORIES.includes(category)) {
    ...
    console.error('Error updating tender expense:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// ─── DELETE /api/accounts/tender-expenses/:id ────────────────────────────────
export const deleteTenderExpense = async (req, res) => {
  try {
    const expense = await TenderExpense.findOne({ _id: req.params.id, companyId: cid(req) });
    if (!expense) return res.status(404).json({ success: false, message: 'Expense not found' });
    ...
    console.error('Error deleting tender expense:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};
