import PurchaseExpense, { PURCHASE_EXPENSE_CATEGORIES } from '../models/PurchaseExpense.js';
...
// ─── GET /api/accounts/purchases/expenses/categories ───────────────────────
export const getPurchaseExpenseCategories = async (req, res) => {
  res.json({ success: true, data: PURCHASE_EXPENSE_CATEGORIES });
};

// ─── POST /api/accounts/purchases/expenses ──────────────────────────────────
export const createPurchaseExpense = async (req, res) => {
  try {
    const { category, amount, date, notes } = req.body;

    if (!category || !PURCHASE_EXPENSE_CATEGORIES.includes(category)) {
    ...
    const expense = await PurchaseExpense.create({
      companyId: cid(req),
      category,
      amount: amountNum,
...
// ─── GET /api/accounts/purchases/expenses ───────────────────────────────────
export const getPurchaseExpenses = async (req, res) => {
  try {
    ...
    const [expenses, total, totalAmountAgg] = await Promise.all([
      PurchaseExpense.find(query)
        .populate('addedBy', 'fullName username')
        .sort({ date: -1, createdAt: -1 })
        .skip(skip)
        .limit(limitNum)
        .lean(),
      PurchaseExpense.countDocuments(query),
      PurchaseExpense.aggregate([
        { $match: query },
...
// ─── GET /api/accounts/purchases/expenses/summary ───────────────────────────
// Category-wise and month-wise totals, for future Purchase Expense Reports.
export const getPurchaseExpenseSummary = async (req, res) => {
  try {
    ...
    console.error('Error fetching purchase expense summary:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// ─── PUT /api/accounts/purchases/expenses/:id ───────────────────────────────
export const updatePurchaseExpense = async (req, res) => {
  try {
    const expense = await PurchaseExpense.findOne({ _id: req.params.id, companyId: cid(req) });
    ...
    if (category !== undefined) {
      if (!PURCHASE_EXPENSE_CATEGORIES.includes(category)) {
    ...
    console.error('Error updating purchase expense:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// ─── DELETE /api/accounts/purchases/expenses/:id ────────────────────────────
export const deletePurchaseExpense = async (req, res) => {
  try {
    const expense = await PurchaseExpense.findOne({ _id: req.params.id, companyId: cid(req) });
    if (!expense) return res.status(404).json({ success: false, message: 'Expense not found' });
    ...
    console.error('Error deleting purchase expense:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};
