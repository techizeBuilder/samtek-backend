...
export const uploadHrPolicyDocument = async (req, res) => {
  try {
    const { policyId } = req.params;
    const file = req.file;
    if (!file) return res.status(400).json({ message: "No file uploaded" });

    const policy = await HrPolicy.findById(policyId);
    if (!policy) return res.status(404).json({ message: "Policy not found" });

    // Remove old file if exists
    if (policy.documentUrl) {
      const oldPath = path.join("uploads/hr-policies", path.basename(policy.documentUrl));
...
export const removeHrPolicyDocument = async (req, res) => {
  try {
    const { policyId } = req.params;
    const policy = await HrPolicy.findById(policyId);
    if (!policy) return res.status(404).json({ message: "Policy not found" });
