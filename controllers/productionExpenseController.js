import ProductionExpense, { PRODUCTION_EXPENSE_CATEGORIES } from '../models/ProductionExpense.js';
...
// ─── GET /api/production/expenses/categories ────────────────────────────────
export const getProductionExpenseCategories = async (req, res) => {
  res.json({ success: true, data: PRODUCTION_EXPENSE_CATEGORIES });
};

// ─── POST /api/production/expenses ──────────────────────────────────────────
export const createProductionExpense = async (req, res) => {
  try {
    const { category, amount, date, notes } = req.body;

    if (!category || !PRODUCTION_EXPENSE_CATEGORIES.includes(category)) {
    ...
    const expense = await ProductionExpense.create({
      companyId: cid(req),
      category,
      amount: amountNum,
    ...
    console.error('Error creating production expense:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// ─── GET /api/production/expenses ───────────────────────────────────────────
export const getProductionExpenses = async (req, res) => {
  try {
    ...
    const [expenses, total, totalAmountAgg] = await Promise.all([
      ProductionExpense.find(query)
        .populate('addedBy', 'fullName username')
        .sort({ date: -1, createdAt: -1 })
        .skip(skip)
        .limit(limitNum)
        .lean(),
      ProductionExpense.countDocuments(query),
      ProductionExpense.aggregate([
        { $match: query },
    ...
    console.error('Error fetching production expenses:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// ─── GET /api/production/expenses/summary ───────────────────────────────────
export const getProductionExpenseSummary = async (req, res) => {
  try {
    ...
    console.error('Error fetching production expense summary:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// ─── PUT /api/production/expenses/:id ────────────────────────────────────────
export const updateProductionExpense = async (req, res) => {
  try {
    const expense = await ProductionExpense.findOne({ _id: req.params.id, companyId: cid(req) });
    ...
    if (category !== undefined) {
      if (!PRODUCTION_EXPENSE_CATEGORIES.includes(category)) {
    ...
    console.error('Error updating production expense:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// ─── DELETE /api/production/expenses/:id ────────────────────────────────────
export const deleteProductionExpense = async (req, res) => {
  try {
    const expense = await ProductionExpense.findOne({ _id: req.params.id, companyId: cid(req) });
    if (!expense) return res.status(404).json({ success: false, message: 'Expense not found' });
    ...
    console.error('Error deleting production expense:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};
