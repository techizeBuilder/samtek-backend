...
const seedDispatchData = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        // Get or create items
        const existingItems = yield db.collection('items').find({ company: company._id }).limit(3).toArray();
        let items = existingItems;
        if (items.length === 0) {
            const timestamp = Date.now();
            items = yield db.collection('items').insertMany([
            ...
            const createdItems = yield db.collection('items').find({ company: company._id }).limit(3).toArray();
            items = createdItems;
        ...
        for (let i = 0; i < 5; i++) {
            for (let j = 0; j < 2; j++) {
                ...
                yield db.collection('dispatches').insertOne(dispatchData);
