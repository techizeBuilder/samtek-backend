...
exports.updateQuestion = exports.deleteQuestion = exports.getQuestions = exports.addQuestionToModule = exports.removeMediaFromModule = exports.addMediaToModule = exports.updateTrainingModule = exports.deactivateModule = exports.getModules = exports.createTrainingModule = exports.deleteTraineeRecord = exports.getDashboardAnalytics = exports.finalizeTrainee = exports.getTraineeDetails = exports.downloadCertificate = exports.getActiveTrainees = exports.updateTraineeModules = exports.stageCandidate = exports.getAvailableTrainees = void 0;
const bcryptjs_1 = __importDefault(require("bcryptjs"));
const TrainingmanagementModel_js_1 = require("../models/TrainingmanagementModel.js");
const trainingEmail_js_1 = require("../utils/trainingEmail.js");
...
const getDepartmentFromRole = (role) => {
    ...
    if (lower.includes('dispatch'))
        return 'Dispatch';
    if (lower.includes('account') || lower.includes('finance'))
        return 'Accounts';
    if (lower.includes('sales'))
        return 'Sales';
    if (lower.includes('research') || lower.includes('r&d'))
...
const getNormalizedDeptPrefix = (role) => {
    ...
    if (lower === 'accounts' || lower === 'finance')
        return 'Account';
    if (lower.includes('research') || lower === 'r&d')
        return 'Research';
    if (lower.includes('quality') || lower === 'qc')
        return 'QC';
    if (lower.includes('store'))
        return 'Store';
    return deptPrefix;
...
const getAvailableTrainees = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const potentialTrainees = yield User_js_1.default.find(query).select('fullName email role username employeeId isTrainee');
        const existingProfiles = yield TrainingmanagementModel_js_1.TrainingProfile.find({ companyId }).select('user');
        const existingUserIds = existingProfiles.map(profile => profile.user.toString());
        const availableToStage = potentialTrainees.filter(trainee => !existingUserIds.includes(trainee._id.toString()));
        res.status(200).json({
            success: true,
...
exports.getAvailableTrainees = getAvailableTrainees;
// --- CONTROLLER: CREATE TRAINEE ---
const stageCandidate = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const traineeUser = yield User_js_1.default.findOne({
            _id: userId,
            companyId,
            isActive: true
        });
        ...
        const existingProfile = yield TrainingmanagementModel_js_1.TrainingProfile.findOne({ user: userId });
        if (existingProfile) {
            return res.status(400).json({
                success: false,
        ...
        const newProfile = yield TrainingmanagementModel_js_1.TrainingProfile.create({
            companyId,
            user: userId,
        ...
        (0, trainingEmail_js_1.sendTrainingEmail)(traineeUser.email, trainingEmail_js_1.TrainingEmailType.CANDIDATE_STAGED, {
...
exports.stageCandidate = stageCandidate;
// --- CONTROLLER: UPDATE TRAINEE MODULES ---
const updateTraineeModules = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        ...
        const profile = yield TrainingmanagementModel_js_1.TrainingProfile.findOneAndUpdate({ _id: id, companyId }, { assignedModules }, { new: true }).populate('assignedModules', 'title category');
        if (!profile) {
...
exports.updateTraineeModules = updateTraineeModules;
// --- CONTROLLER: GET ACTIVE TRAINEES ---
const getActiveTrainees = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { companyId, role: managerRole } = req.user;
        let query = { companyId };
        if (TOP_LEVEL_ADMINS.includes(managerRole)) {
        ...
        if (req.query.search) {
            ...
            const matchingUsers = yield User_js_1.default.find({
                companyId,
                $or: [
        ...
        const profiles = yield TrainingmanagementModel_js_1.TrainingProfile.find(query)
            .populate('user', 'fullName email role employeeId')
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limit)
            .lean();
        const totalDocuments = yield TrainingmanagementModel_js_1.TrainingProfile.countDocuments(query);
        res.status(200).json({
            success: true,
...
exports.getActiveTrainees = getActiveTrainees;
...
const downloadCertificate = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const profile = yield TrainingmanagementModel_js_1.TrainingProfile.findOne({ _id: id, companyId }).populate('user', 'fullName').lean();
        if (!profile)
            return res.status(404).json({ success: false, message: "Profile not found." });
        ...
        const doc = new pdfkit_1.default({ size: 'A4', layout: 'landscape', margins: { top: 50, bottom: 50, left: 50, right: 50 } });
        doc.pipe(res);
        // A. Draw Border
        doc.rect(30, 30, doc.page.width - 60, doc.page.height - 60).lineWidth(3).stroke('#1e3a8a');
        // B. Add Samtek Logo
        const logoPath = path_1.default.resolve(process.cwd(), 'assets', 'logo Semtek.png');
...
const getTraineeDetails = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const { companyId } = req.user;
        const profile = yield TrainingmanagementModel_js_1.TrainingProfile.findOne({ _id: id, companyId })
        ...
        const userProgress = yield TrainingmanagementModel_js_1.Progress.find({ user: profile.user._id }).lean();
        const testAttempts = yield TrainingmanagementModel_js_1.TestAttempt.find({ user: profile.user._id })
        ...
        const detailedModules = profile.assignedModules.map(module => {
            ...
            const modTests = testAttempts
                ...
                const reviewData = t.randomizedQuestionSet ? t.randomizedQuestionSet.map(q => {
                    var _a;
                    const userAns = (_a = t.submittedAnswers) === null || _a === void 0 ? void 0 : _a.find(a => a.questionId.toString() === q._id.toString());
                ...
                return Object.assign(Object.assign({}, t), { timeTakenFormatted,
                    totalQuestions,
                    correctAnswers,
                    wrongAnswers,
                    reviewData });
            ...
            if (status === 'Completed')
                completedCount++;
            return Object.assign(Object.assign({}, module), { progressStatus: status, isTestUnlocked: modProgress ? modProgress.isTestUnlocked : false, testAttempts: modTests });
        });
        ...
        res.status(200).json({
            success: true,
            data: Object.assign(Object.assign({}, profile), { assignedModules: detailedModules, analytics: {
                    totalAssigned,
                    completedCount,
                    progressPercentage
                } })
...
exports.getTraineeDetails = getTraineeDetails;
// --- CONTROLLER: FINALIZE TRAINEE (Hire or Reject) ---
const finalizeTrainee = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const { action } = req.body;
        const profile = yield TrainingmanagementModel_js_1.TrainingProfile.findById(id).populate('user');
        if (!profile || !profile.user) {
            return res.status(404).json({ success: false, message: "Trainee profile not found." });
        }
        if (action === 'hire') {
            yield User_js_1.default.findByIdAndUpdate(profile.user._id, {
                isTrainee: false,
                isActive: true
            });
            profile.status = 'Completed_Onboarding';
            yield profile.save();
            (0, trainingEmail_js_1.sendTrainingEmail)(profile.user.email, trainingEmail_js_1.TrainingEmailType.TRAINING_PASSED, {
            ...
            yield User_js_1.default.findByIdAndUpdate(profile.user._id, { isActive: false });
            profile.status = 'Rejected';
            profile.isEligible = false;
            yield profile.save();
            (0, trainingEmail_js_1.sendTrainingEmail)(profile.user.email, trainingEmail_js_1.TrainingEmailType.TRAINING_REJECTED, {
                fullName: profile.user.fullName,
                department: profile.assignedDepartment
            });
            return res.status(200).json({
...
exports.finalizeTrainee = finalizeTrainee;
...
const getDashboardAnalytics = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const profiles = yield TrainingmanagementModel_js_1.TrainingProfile.find(profileQuery).populate('user', 'fullName').lean();
        ...
        profiles.forEach(p => {
            ...
            if (!departmentStats[dept])
                departmentStats[dept] = { total: 0, passed: 0 };
            departmentStats[dept].total += 1;
            if (p.status === 'Passed' || p.status === 'Completed_Onboarding')
                departmentStats[dept].passed += 1;
            if (p.user)
                userDeptMap[p.user._id.toString()] = dept;
        });
        const validUserIds = profiles.map(p => { var _a; return (_a = p.user) === null || _a === void 0 ? void 0 : _a._id; }).filter(Boolean);
        const attempts = yield TrainingmanagementModel_js_1.TestAttempt.find({ companyId, user: { $in: validUserIds } })
                    ...
                    if (ans.isCorrect === false) {
                        ...
                        if (!failedQuestionsTracker[qId]) {
                            failedQuestionsTracker[qId] = {
                                count: 0,
                                module: ((_a = attempt.module) === null || _a === void 0 ? void 0 : _a.title) || 'Unknown',
                                department: ((_b = attempt.module) === null || _b === void 0 ? void 0 : _b.department) || 'Unknown'
                            };
                        }
        ...
        const topFailedIds = Object.keys(failedQuestionsTracker)
            ...
            .slice(0, 5);
        const failedQuestionsData = yield TrainingmanagementModel_js_1.Question.find({ _id: { $in: topFailedIds } }).select('questionText').lean();
...
const deleteTraineeRecord = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const profile = yield TrainingmanagementModel_js_1.TrainingProfile.findById(id);
        if (!profile) {
            return res.status(404).json({ success: false, message: "Profile not found." });
        }
        const userId = profile.user;
        yield TrainingmanagementModel_js_1.Progress.deleteMany({ user: userId });
        yield TrainingmanagementModel_js_1.TestAttempt.deleteMany({ user: userId });
        yield TrainingmanagementModel_js_1.TrainingProfile.findByIdAndDelete(id);
...
exports.deleteTraineeRecord = deleteTraineeRecord;
// --- CONTROLLER: CREATE TRAINING MODULE ---
const createTrainingModule = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const newModule = yield TrainingmanagementModel_js_1.Module.create({
            companyId,
            title,
            description,
...
exports.createTrainingModule = createTrainingModule;
const getModules = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const modules = yield TrainingmanagementModel_js_1.Module.find(query).sort({ sequenceOrder: 1 });
...
const deactivateModule = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const updatedModule = yield TrainingmanagementModel_js_1.Module.findOneAndUpdate({ _id: req.params.id, companyId: req.user.companyId }, { isActive: false }, { new: true });
        if (!updatedModule)
...
const updateTrainingModule = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        ...
        const updatedModule = yield TrainingmanagementModel_js_1.Module.findOneAndUpdate({ _id: id, companyId }, { title, description, category, sequenceOrder: parseInt(sequenceOrder) }, { new: true, runValidators: true });
        if (!updatedModule) {
            return res.status(404).json({ success: false, message: "Module not found." });
        }
...
exports.updateTrainingModule = updateTrainingModule;
const addMediaToModule = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const { companyId, role } = req.user;
        if (!TOP_LEVEL_ADMINS.includes(role) && !DEPT_HEADS.includes(role)) {
        ...
        const module = yield TrainingmanagementModel_js_1.Module.findOne({ _id: id, companyId });
        if (!module) {
            return res.status(404).json({ success: false, message: "Module not found." });
        }
        if (!req.files || req.files.length === 0) {
...
exports.addMediaToModule = addMediaToModule;
const removeMediaFromModule = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const module = yield TrainingmanagementModel_js_1.Module.findOne({ _id: moduleId, companyId });
        if (!module) {
            return res.status(404).json({ success: false, message: "Module not found." });
        }
        ...
        try {
            const filePath = path_1.default.resolve(mediaItem.mediaUrl);
...
exports.removeMediaFromModule = removeMediaFromModule;
// --- CONTROLLER: ADD QUESTION TO MODULE ---
const addQuestionToModule = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { moduleId } = req.params;
        const { questionText, options, correctOption } = req.body;
        const { companyId, _id: authorityId } = req.user;
        const parentModule = yield TrainingmanagementModel_js_1.Module.findOne({ _id: moduleId, companyId });
        if (!parentModule) {
        ...
        const newQuestion = yield TrainingmanagementModel_js_1.Question.create({
            companyId,
            module: moduleId,
            questionText,
            options,
...
exports.addQuestionToModule = addQuestionToModule;
const getQuestions = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const questions = yield TrainingmanagementModel_js_1.Question.find(query).skip(skip).limit(limit);
        const totalDocuments = yield TrainingmanagementModel_js_1.Question.countDocuments(query);
        res.status(200).json({
...
const deleteQuestion = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const deletedQuestion = yield TrainingmanagementModel_js_1.Question.findOneAndDelete({ _id: req.params.questionId, companyId: req.user.companyId });
        if (!deletedQuestion)
...
const updateQuestion = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const updatedQuestion = yield TrainingmanagementModel_js_1.Question.findOneAndUpdate({ _id: questionId, companyId }, { questionText, options, correctOption }, { new: true, runValidators: true });
        if (!updatedQuestion) {
            return res.status(404).json({ success: false, message: "Question not found." });
        }
