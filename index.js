...
(async () => {
    try {
        ...
        app.get('/api/fix-sales-permissions', async (req, res) => {
            try {
                const User = (await import('./models/User.js')).default;
                const result = await User.updateMany({ role: 'Sales' }, {
                    $set: {
                        'permissions.modules': [
                            {
                                name: 'sales',
        ...
        app.get('/api/fix-production-permissions', async (req, res) => {
            try {
                const { updateUsersWithProductionPermissions, migrateOldProductionPermissions } = await import('./utils/updateProductionPermissions.js');
