...
async function test() {
  ...
  const leads = await Lead.find({}, { _id: 1, quotation: 1, profile: 1, contacts: 1, address: 1 }).sort({ createdAt: -1 }).limit(20);
  let totalSize = 0;
  leads.forEach((l, i) => {
    const size = Buffer.byteLength(JSON.stringify(l), 'utf8');
    totalSize += size;
    if (size > 1024 * 1024) console.log('Lead', i, 'Size:', (size / 1024 / 1024).toFixed(2), 'MB');
  });
