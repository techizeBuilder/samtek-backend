...
async function restoreOriginalStoreFields() {
  try {
    ...
    const restoreResult = await Item.updateMany(
      {
    ...
    const newResult = await Item.updateMany(
      {
    ...
    // Show final status
    const allProductsWithStore = await Item.find({ 
    ...
    const productsWithoutStore = await Item.find({ 
      $or: [
        { store: { $exists: false } },
        { store: null },
        { store: "" }
